    ...
    def consume(self, resource: str, amount: int) -> tuple[bool, str]:
        ...
        with self._lock:
            ...
            if current_val + amount > max_val:
                return False, f"{resource} limit exceeded: {current_val + amount} > {max_val}"
        ...
        if os.path.exists(self.budget_file):
            with open(self.budget_file, 'r') as f:
                ...
                for date_str, budget_data in data.items():
                    ...
                    # Filter out keys that might not exist in DailyBudget fields if structure changed
                    valid_keys = DailyBudget.__annotations__.keys()
                    filtered_data = {k: v for k, v in budget_data.items() if k in valid_keys}
