...
class ResearchState(DictCompatibleModel):
    ...
    # Use List[dict] instead of List[Citation] to avoid JSON serialization issues
    # Citation objects are converted to dicts before being stored in state
    citations: List[dict] = Field(
        default_factory=list, description="Final list of citations (as dicts)"
    )
    ...
    # Approach evaluation and selection (for human-in-the-loop)
    approach_evaluation: Optional[ApproachEvaluation] = Field(
        default=None,
        description="Evaluation of multiple research approaches (generated by approach_evaluator)"
    )
    selected_approach_index: Optional[int] = Field(
        default=None,
        ge=0,
        le=2,
        description="Index (0-2) of the selected approach (set by human_approach_selector)"
    )
    selected_approach: Optional[str] = Field(
        default=None,
        description="Description of the selected approach"
    )
    selection_reasoning: Optional[str] = Field(
        default=None,
        description="Reasoning for why this approach was selected"
    )
    awaiting_human_input: bool = Field(
        default=False,
        description="Whether the system is currently awaiting human input"
    )
...
class LeadResearcherState(DictCompatibleModel):
    ...
    # Approach selection fields (set by human_approach_selector)
    selected_approach: Optional[str] = Field(
        default=None,
        description="Description of the selected approach"
    )
    ...
    approach_evaluation: Optional[ApproachEvaluation] = Field(
        default=None,
        description="Evaluation of multiple research approaches"
    )
...
class CitationAgentState(DictCompatibleModel):
    ...
    subagent_findings: List[Finding] = Field(..., min_length=1)
    synthesized_results: str = Field(..., min_length=1)
    # Use List[dict] instead of List[Citation] to avoid serialization issues
    all_extracted_citations: List[dict] = Field(
